  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const assignedTeacherIds = new Set(classResult.class.teachers.map((t: Teacher) => t._id));
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {classData.className} {classData.section} - Grade {classData.grade.gradeNumber}
          </CardTitle>
          <CardDescription>
            Assign teachers to this class for the {classData.academicYear} academic year.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {teachers.length === 0 ? (
            <div className="text-center py-6">
              <p className="text-muted-foreground">No teachers found in the system.</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
